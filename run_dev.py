...
try:
    ...
    # 如果直接导入失败，尝试通过 src.backend 导入
    try:
        from src.backend import backend_init
    except ImportError as e:
        print(f"Error importing backend_init: {e}")
...
def start_backend_process():
    ...
    # 注意：这里 app 的导入路径需要根据实际情况调整，如果运行 run_dev.py 时 cwd 是项目根目录，
    # 且 src/backend 在 sys.path 中，则可能是 "app:app" 或者 "src.backend.app:app"
    cmd = [
